...
export function integrationEvent(program: Command) {
  return program
    ...
    .action(async (organizationId, integrationId, directory) => {
      const result = await integrationMetaRepository.integrationEvents(organizationId, integrationId);
      if (result.onSuccess) {
        console.log("=========================================================================================");
  ...
  return program
    ...
    .action(async (organizationId, integrationId, directory) => {
      try {
        ...
        const result = await integrationMetaRepository.syncIntegrationEvents(organizationId, integrationId, json);
        if (result.onSuccess) {
          console.log("=========================================================================================");
          fs.writeFileSync(`${path}/events.json`, JSON.stringify(result.onSuccess));
