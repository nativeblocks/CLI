...
export function integrationProperty(program: Command) {
  return program
    ...
    .action(async (organizationId, integrationId, directory) => {
      const result = await integrationMetaRepository.integrationProperties(organizationId, integrationId);
      if (result.onSuccess) {
        console.log("=========================================================================================");
  ...
  return program
    ...
    .action(async (organizationId, integrationId, directory) => {
      try {
        ...
        const result = await integrationMetaRepository.syncIntegrationProperties(organizationId, integrationId, json);
        if (result.onSuccess) {
          console.log("=========================================================================================");
          fs.writeFileSync(`${path}/properties.json`, JSON.stringify(result.onSuccess));
